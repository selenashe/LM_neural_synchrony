#!/usr/bin/env bash
#SBATCH --job-name=ll_fb_gemini
#SBATCH --account=nlp
#SBATCH --output=/juice6/u/jshe/nlp/LM_neural_synchrony/logs/logit_lens_fb_gemini_%j.out
#SBATCH --error=/juice6/u/jshe/nlp/LM_neural_synchrony/logs/logit_lens_fb_gemini_%j.err
...
# No array needed — one job processes all 600 episodes.
...
pair_dir="${REPO_ROOT}/sotopia_results_false_belief_100_gemini/dialogs/${model_1}_None_0_${model_2}_None_0_false_belief_fixed_two_agents"
...
python analysis/setup_logit_lens_false_belief.py \
    --model_1 "${model_1}" \
    --model_2 "${model_2}" \
    --results_dir sotopia_results_false_belief_100_gemini \
    --output_dir logit_lens_results_false_belief_100_gemini
