...
def _parse_model_short_name(model_name_all: str) -> str:
    """Extract the model_paths key from Sotopia names like Mistral-7B-Instruct-v0.3_None_0."""
    parts = model_name_all.rsplit("_", 2)
    ...
    # Fallback: try the full name (covers names like allenai_OLMo-3-7B-Instruct_None_0
    # where rsplit didn't match the _None_0 pattern above — shouldn't happen,
    # but also try first segment as last resort).
    return model_name_all.split("_")[0]


def getLM(model_name_all, model_paths, temperature, seed):
    """Load model by short name extracted from the Sotopia naming convention."""
    model_name = _parse_model_short_name(model_name_all)
    if not model_name:
        raise KeyError(
            f"Empty model short name after parsing {model_name_all!r} "
            "(check SLURM pair arrays / --model_1 --model_2)."
        )
    if model_name not in model_paths:
        raise KeyError(
            f"Model short name {model_name!r} (from {model_name_all!r}) "
            f"not found in model_paths.json. Available: {sorted(model_paths.keys())}"
        )
...
class SotopiaEnv():
    ...
    def _act(self, actor: int, save_states: bool = False, turn: int = 0):
        ...
        self.thinking_traces.append(now_model._last_thinking_trace)
...
def random_sample_agents_from_pool_with_descriptions_standalone(agents_file_path=None,
    ...
    if agents_file_path is None:
        agents_file_path = os.path.join(REPO_ROOT, 'sotopia_utils/sotopia_data/agents.json')
