#!/usr/bin/env python3
"""
V5.0 — Sycophancy MMLU pilot.

Same 2×2×2 factorial as v4.5 (turns × role × framing), applied to sycophancy
on MMLU factual knowledge questions.
...
20 MMLU questions × 10 sycophancy cues (strength 1–10) = 200 trials per
condition.  Four-choice (A/B/C/D) MMLU format; option order randomized per
trial.  Measures flip rate: whether the model changes from the correct answer
to the suggested wrong answer under sycophantic pressure.

Usage:
  python simulation/sample_olmo3_checkpoints_v5_0_sycophancy.py \\
      --checkpoint_name Olmo-3.1-32B-Instruct \\
      --condition v5_0_single_sys_quote --dry_run

  python simulation/sample_olmo3_checkpoints_v5_0_sycophancy.py \\
      --checkpoint_name Olmo-3.1-32B-Instruct \\
      --condition v5_0_multi_uo_report
"""
...
def _results_dir(name, condition, label=""):
    ...
    return f"sotopia_results_{family}_v5_0_{suffix}_pilot{label_suffix}"


# ── Data loading ───────────────────────────────────────────────────────────

def load_envs(envs_basename="envs_sycophancy_mmlu_v5_0_pilot.json"):
    with open(SOTOPIA_DATA / envs_basename) as f:
        return json.load(f)


def load_combos(envs_basename="envs_sycophancy_mmlu_v5_0_pilot.json"):
...
def build_option_order(meta, episode_idx, seed):
    """Shuffle the 4 MMLU options. Returns (correct_pos, wrong_pos, [opt_a..d])."""
...
def build_choice_block(question_text, options):
    return (
        f"{question_text}\n\n"
        f"(A) {options[0]}\n"
        f"(B) {options[1]}\n"
...
def format_single_prompt(meta, shuffled_options, wrong_pos, condition, checkpoint_name):
    ...
    qblock = build_choice_block(meta["question_text"], shuffled_options)
...
def run_dry(model, tokenizer, checkpoint_name, condition, envs, combos,
    ...
    for ep_idx, combo in enumerate(combos[:6]):
        ...
        print(f"\n{'='*80}")
        print(f"[{ep_idx+1}] {meta['question_id']} × {meta['cue_id']}  "
              f"strength={meta['cue_strength']}  domain={meta['question_domain']}")
...
def save_prompt_record(results_dir, checkpoint_name, seed, ep_idx, env_id,
    ...
    header = (
        ...
        f"question_id={meta['question_id']}  cue_id={meta['cue_id']}  "
        f"cue_strength={meta['cue_strength']}  ({meta['cue_strength_label']})\n"
    ...
    with open(csv_path, "a", newline="") as f:
        ...
        for ep_idx in range(total):
            ...
            row = {
                ...
                "question_domain": meta["question_domain"],
                "cue_id": meta["cue_id"],
...
def main():
    parser = argparse.ArgumentParser(
        description="V5.0: Sycophancy MMLU pilot (2x2x2 factorial)."
    )
    ...
    parser.add_argument("--envs", type=str,
                        default="envs_sycophancy_mmlu_v5_0_pilot.json")
