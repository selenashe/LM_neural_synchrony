#!/usr/bin/env python3
"""
Single-prompt completion evaluation of OLMo-3 checkpoints on
false-belief v3 scenarios.

Supports two training tracks:
  - Instruct: Base -> SFT -> DPO -> RLVR-step400
  - Think:    SFT -> DPO -> RLVR-step1375  (reasoning models with <think> blocks)

Think models produce <think>...</think> reasoning blocks which are stripped
before parsing the actual dialogue response.

Usage:
  # Dry run (24 episodes, one per condition, prints prompt + response):
  python simulation/sample_olmo3_checkpoints.py \
      --checkpoint_name OLMo-3-7B-Instruct-SFT --dry_run

  # Full run:
  python simulation/sample_olmo3_checkpoints.py \
      --checkpoint_name OLMo-3-7B-Think-RLVR-step1375
"""
...
ALL_CHECKPOINTS = INSTRUCT_CHECKPOINTS + THINK_CHECKPOINTS

BASE_CHECKPOINT = "OLMo-3-1025-7B"

INSTRUCT_RESULTS_DIR = "sotopia_results_olmo3_instruct_checkpoints_v3"
THINK_RESULTS_DIR = "sotopia_results_olmo3_think_checkpoints_v3"


def _is_think_checkpoint(name):
    return name in THINK_CHECKPOINTS


def _is_base_checkpoint(name):
    return name == BASE_CHECKPOINT


def _results_dir_for(name):
    if _is_think_checkpoint(name):
        return THINK_RESULTS_DIR
    return INSTRUCT_RESULTS_DIR
...
def format_prompt(raw_prompt, tokenizer, checkpoint_name):
    """Wrap the raw prompt with chat template."""
    if _is_base_checkpoint(checkpoint_name):
        return raw_prompt

    if _is_think_checkpoint(checkpoint_name):
        return (
            f"<|im_start|>user\n{raw_prompt}<|im_end|>\n"
...
def strip_think_blocks(text):
    """Remove <think>...</think> reasoning blocks from Think model output.

    The prompt already includes the opening <think>, so the generated text
    starts inside the think block.  The model emits </think> when done
    reasoning, followed by the actual response.
    """
...
def parse_ava_line(raw_response, is_think):
...
def run_dry(model, tokenizer, checkpoint_name, envs, combos, scenario_vocab,
            temperature, seed):
    """Run one episode per condition (24 total), print prompt + response."""
    is_think = _is_think_checkpoint(checkpoint_name)
    max_tokens = 2000 if is_think else 500
    seen_conditions = set()
    count = 0
    ...
    for combo in combos:
        ...
        formatted = format_prompt(raw_prompt, tokenizer, checkpoint_name)
        ...
        ava_line = parse_ava_line(raw_response, is_think)

        print(f"RAW RESPONSE:\n{raw_response}")
        if is_think:
            print(f"{'─'*80}")
            print(f"AFTER STRIPPING THINK: {strip_think_blocks(raw_response)}")
        print(f"{'─'*80}")
...
def run_full(model, tokenizer, checkpoint_name, envs, combos, scenario_vocab,
             temperature, seed, max_episodes=None):
    is_think = _is_think_checkpoint(checkpoint_name)
    max_tokens = 2000 if is_think else 500

    results_dir = REPO_ROOT / _results_dir_for(checkpoint_name)
    ...
    print(f"  Track: {'Think' if is_think else 'Instruct'}, "
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate OLMo-3 checkpoints on false-belief v3 scenarios."
    )
    parser.add_argument(
        "--checkpoint_name", required=True, choices=ALL_CHECKPOINTS,
        help="Which OLMo-3 checkpoint to evaluate.",
    )
    parser.add_argument(
    ...
    if args.dry_run:
        run_dry(
            ...
            scenario_vocab, args.temperature, args.seed,
        )
    else:
        run_full(
            model, tokenizer, args.checkpoint_name, envs, combos,
