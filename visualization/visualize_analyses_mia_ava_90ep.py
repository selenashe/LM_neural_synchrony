#!/usr/bin/env python3
"""
Mia + Ava, false-belief (600-episode) analysis plots (same logic as visualize_analyses_mia_ava_90ep.ipynb).
...
Section E prefers ``control_analysis_summary_false_belief_600ep.json`` when present; it does not load
...
from core.utils import get_short_names_and_identifiers
...
SBERT_OUTPUT_TAG = "false_belief_600ep"
...
def plot_affine_heatmaps():
    ...
    out = os.path.join(SAVE_DIR, "A1_layer_heatmap_false_belief_600ep.png")
...
def plot_affine_heatmaps_by_goal():
    ...
    out = os.path.join(SAVE_DIR, "A1b_layer_heatmap_by_goal_false_belief_600ep.png")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {out}")
...
def plot_sbert_controls():
    ...
    if sbert_path is None:
        raise FileNotFoundError(
            ...
            "--results_postfix _false_belief_fixed_two_agents --output_tag false_belief_600ep"
        )
    ...
    out = os.path.join(SAVE_DIR, "A2_sbert_controls_grouped_false_belief_600ep.png")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved {out}")
