#!/usr/bin/env python3
"""
Aggregate and plot summary results from the false-belief v2 logit-lens experiments.

V2 uses redesigned conditions (C1-C6):
  C1: mutual_knowledge + cooperate
  C2: asymmetric_knowledge + cooperate
  C3: asymmetric_knowledge + adversarial_warned
  C4: conflicting_evidence + cooperate
  C5: conflicting_evidence + adversarial_warned
  C6: mutual_knowledge + adversarial_incoherence
...
RESULTS_ROOT = REPO_ROOT / "logit_lens_results_false_belief_v2"
OUT_DIR = REPO_ROOT / "summary_plots_false_belief_v2"

PAIR_DIRS = sorted(RESULTS_ROOT.iterdir()) if RESULTS_ROOT.exists() else []

CONDITIONS = [
    "mutual_coop", "mutual_incoh",
    "asym_coop", "asym_adv",
    "conflict_coop", "conflict_adv",
]

CONDITION_LABELS = {
    "mutual_coop": "Mutual\nCooperate",
    "mutual_incoh": "Mutual\nAdv-Incoh",
    "asym_coop": "Asymmetric\nCooperate",
    "asym_adv": "Asymmetric\nAdv-Warned",
    "conflict_coop": "Conflict\nCooperate",
    "conflict_adv": "Conflict\nAdv-Warned",
}
...
GOAL_SHORT = {
    "cooperate": "coop",
    "adversarial_warned": "adv",
    "adversarial_incoherence": "incoh",
}
...
with open(SOTOPIA_DATA_DIR / "envs_false_belief_v2.json", encoding="utf-8") as _f:
    _ENVS = json.load(_f)
...
for _env in _ENVS.values():
    ...
    _belief_s = BELIEF_SHORT[_belief]
    _goal_s = GOAL_SHORT[_goal]
    _cond = f"{_belief_s}_{_goal_s}"
    _suffix = f"_{_belief_s}_{_goal_s}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Aggregate and plot false-belief v2 experiment results.")
    parser.add_argument("--results_dir", type=str, default="logit_lens_results_false_belief_v2",
                        help="Results directory name (relative to REPO_ROOT).")
    parser.add_argument("--output_dir", type=str, default="summary_plots_false_belief_v2",
