#!/usr/bin/env python3
"""
Conversation quality audit for false-belief v2 dialog experiments.
...
OUT_DIR = REPO_ROOT / "summary_plots_false_belief_v2" / "quality_audit"
...
def detect_guide_goal_failure(d: Dict) -> bool:
    ...
    if goal == "cooperate" and has_false and not has_true:
        if _is_negated(false_display.lower(), text):
            return False
        return True
    if goal in ("adversarial_warned", "adversarial_incoherence") and has_true and not has_false:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Conversation quality audit for false-belief v2 experiments.")
    ...
    with open(report_path, "w") as rpt:
        rpt.write("CONVERSATION QUALITY AUDIT REPORT (V2)\n")
        rpt.write("=" * 60 + "\n")
