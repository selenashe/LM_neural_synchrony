...
Target case types:
  ...
  2. mutual_incoh_fail      — both know truth, guide incoherent, seeker picks wrong
  ...
  6. mutual_incoh_success   — both know truth, guide incoherent, seeker picks correctly
...
DIALOGS_ROOT = REPO_ROOT / "sotopia_results_false_belief_v2" / "dialogs"
OUT_DIR = REPO_ROOT / "summary_plots_false_belief_v2" / "error_cases"
...
CASE_TYPES = {
    ...
    "mutual_incoh_fail": {"belief": "mutual_knowledge", "goal": "adversarial_incoherence", "correct": False},
    "conflict_coop_success": {"belief": "conflicting_evidence", "goal": "cooperate", "correct": True},
    "conflict_adv_success": {"belief": "conflicting_evidence", "goal": "adversarial_warned", "correct": True},
    "conflict_coop_fail": {"belief": "conflicting_evidence", "goal": "cooperate", "correct": False},
    "mutual_incoh_success": {"belief": "mutual_knowledge", "goal": "adversarial_incoherence", "correct": True},
}
...
def parse_pair(pair_dir_name: str) -> Tuple[str, str]:
    ...
    m1 = parts[0]
    m2 = parts[1].split("_false_belief_v2")[0]
    return m1, m2
...
# ---------------------------------------------------------------------------
# Lightweight quality flags (subset of audit detectors)
# ---------------------------------------------------------------------------
...
def check_flags(d: Dict) -> Dict[str, bool]:
    ...
    if d["turns"] and d["turns"][0]["speaker"] == "Mia Sanders":
        ...
        if meta["goal_condition"] in ("adversarial_warned", "adversarial_incoherence") and has_true and not has_false:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract surprising behavioral outcomes for inspection (v2).")
    ...
    with open(report_path, "w") as rpt:
        rpt.write("ERROR CASES REPORT (V2)\n")
        rpt.write("=" * 60 + "\n\n")
