...
Target case types:
  1. shared_help FAIL     — both know truth, guide helps, yet seeker picks wrong
...
DIALOGS_ROOT = REPO_ROOT / "sotopia_results_false_belief_100" / "dialogs"
OUT_DIR = REPO_ROOT / "summary_plots_false_belief_100" / "error_cases"
...
CASE_TYPES = {
    "shared_help_fail": {"belief": "shared_truth", "goal": "help", "correct": False},
...
def check_flags(d: Dict) -> Dict[str, bool]:
    ...
    if d["turns"] and d["turns"][0]["speaker"] == "Mia Sanders":
        ...
        if meta["goal_condition"] == "help" and has_false and not has_true:
            if not _is_negated(false_display, text):
                flags["guide_goal_failure"] = True
        if meta["goal_condition"] == "deceive" and has_true and not has_false:
...
def format_case_transcript(d: Dict, ep_dir_name: str, ep_idx: int,
    ...
    lines.append(
        ...
        f"Ava's belief: {meta['b_belief'].replace('_', ' ')} | "
...
def main():
    ...
    with open(report_path, "w") as rpt:
        ...
        # Top failure-prone pairs per case type
